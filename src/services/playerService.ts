...
class PlayerService {
  ...
  private attachEventListeners() {
    ...
    const _playbackStateListener = TrackPlayer.addEventListener(Event.PlaybackState, event => {
      const isPlaying = event.state === State.Playing;
      const isLoading = event.state === State.Buffering || event.state === State.Loading;
      this.updateState({ isPlaying, isLoading });
    });

    const _progressListener = TrackPlayer.addEventListener(Event.PlaybackProgressUpdated, event => {
      this.updateState({ position: event.position, duration: event.duration });
    });

    const _trackChangedListener = TrackPlayer.addEventListener(
    ...
    try {
      ...
      if (prevItems.length > 0) {
        await TrackPlayer.add(
          prevItems.map(t => this.toTPTrack(t)),
          0
        );
      }
    } catch (e) {
