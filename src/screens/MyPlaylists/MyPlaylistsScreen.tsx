...
import { Surface, Text, ActivityIndicator, useTheme, Button, Card, Avatar, IconButton } from 'react-native-paper';
...
const MyPlaylistsScreen: React.FC<MyPlaylistsScreenProps> = ({ navigation }) => {
    ...
    const fetchUserPlaylists = async (showRefreshing = false, retryToken?: string) => {
        try {
            ...
            if ((err.message.includes('Invalid or expired token') || err.message.includes('401')) && !retryToken) {
                try {
        ...
        <View style={styles.header}>
            <View style={styles.headerRow}>
                <View style={styles.headerText}>
                    <Text variant="headlineMedium" style={styles.headerTitle}>Cloud Playlists</Text>
    ...
    if (error && !playlists.length) {
        return (
            <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={['top', 'left', 'right']}>
                <View style={styles.centerContent}>
                    <Avatar.Icon size={64} icon="alert-circle" style={{ marginBottom: 16, backgroundColor: theme.colors.errorContainer }} color={theme.colors.error} />
                    <Text variant="headlineSmall" style={styles.emptyTitle}>
